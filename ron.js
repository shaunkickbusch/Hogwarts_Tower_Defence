class Ron{
  ...
  constructor(positionX, positionY){
    ...
    this.attack = new Projectile(positionX, positionY, 0, color("#FFFF00"), RON_ATTACK_SPEED);
    this.currentEnemyIndex = 0;
    this.areWeAttacking = false;
    this.sellPrice = RON_SELL_PRICE;
  }
  ...
  attackMove(){
    ...
    if (!(frameCount % 10)){
      this.attackingFrameCounter = (this.attackingFrameCounter + 1) % ronAttack.length;
    }
    ...
    if(enemies.length > 0){
      ...
      if(this.attack.successfullHit === true){
        ...
        if(enemies[this.currentEnemyIndex].hasDied === true){
          ...
          this.attack = new Projectile(this.pos.x, this.pos.y, 0, color("#FFFF00"), RON_ATTACK_SPEED);
        }
      }
    }
  }
  ...
  attackDisplay(){
    if(enemies.length > 0){
      this.attack.display();
      image(ronAttack[this.attackingFrameCounter], this.pos.x, this.pos.y);
    }
  }

  moveIdle(){
    if (!(frameCount % 10)){
      this.idleFrameCounter = (this.idleFrameCounter + 1) % ronIdle.length;
    }
  }

  displayIdle(){
    image(ronIdle[this.idleFrameCounter], this.pos.x, this.pos.y);
  }
}
